...
struct PlayerModelRegressionTests {
    ...
    @Test
    func listeningHistoryUploadsEveryProfileAndMergesOnlyTheActiveProfile() async throws {
        let (defaults, suiteName) = try isolatedDefaults()
        defer { defaults.removePersistentDomain(forName: suiteName) }
        ...
        await initial.importLocalFiles(at: [hero])
        let importedTrack = try #require(initial.tracks.first)
        initial.selectAndPlay(importedTrack)
        try await Task.sleep(for: .milliseconds(200))
        initial.togglePlay()
        let localEntry = try #require(initial.listeningHistoryEntries.first)
        let otherProfileEntry = ListeningHistoryEntry(
            trackID: UUID(),
        ...
        defaults.set(
            try JSONEncoder().encode([localEntry, otherProfileEntry]),
            forKey: "LikedSongsFocus.listeningHistory.v1"
        )
        ...
        let remoteOnlyEventID = "windows-history-event"
        var postCount = 0
        var getCount = 0
        var uploadedProfileIDs = Set<String>()
        RegressionURLProtocol.handler = { request in
            ...
            if request.httpMethod == "POST" {
                ...
                let profileID = try #require(
                    request.value(forHTTPHeaderField: "X-Resonance-Profile")
                )
                uploadedProfileIDs.insert(profileID)
                let body = try #require(
                    JSONSerialization.jsonObject(with: regressionRequestBody(request))
                        as? [String: Any]
                )
                let entries = try #require(body["entries"] as? [[String: Any]])
                #expect(entries.count == 1)
                if profileID == "default" {
                    #expect(entries.first?["id"] as? String == localEntry.networkEventID)
                    #expect(entries.first?["song_id"] as? String == "shared-song")
                } else {
                    #expect(profileID == "other-profile")
                    #expect(entries.first?["id"] as? String == otherProfileEntry.networkEventID)
                }
                #expect(body["client"] as? String == "macos")
                return (
                    try response(for: request),
                    try JSONSerialization.data(withJSONObject: ["accepted": entries.count])
                )
            }

            getCount += 1
            #expect(request.value(forHTTPHeaderField: "X-Resonance-Profile") == "default")
            #expect(request.url?.query == "limit=2000")
            let payload: [String: Any] = [
                "profile_id": "default",
                "entries": [
                    [
                        "id": localEntry.networkEventID,
                        "track_id": localEntry.trackID.uuidString,
                        "song_id": "shared-song",
                        "started_at": "2026-08-02T17:00:00.000Z",
                        "listened_seconds": 45,
                        "title": "Shared title",
                    ...
                    [
                        "id": remoteOnlyEventID,
                        "track_id": "windows-track-id",
                        "song_id": "remote-only-song",
                        "started_at": "2026-08-02T18:00:00Z",
                        "listened_seconds": 90,
                        "title": "Remote-only title",
                        "artist": "Remote-only artist",
                        "album": "Remote-only album",
        ...
        let localTrackIndex = try #require(model.tracks.firstIndex(where: { $0.id == importedTrack.id }))
        model.tracks[localTrackIndex].remoteID = "shared-song"
        model.tracks[localTrackIndex].syncProfileID = "default"
        model.serverURLString = "https://music.test"
        model.serverToken = "access-token"
        ...
        #expect(postCount == 2)
        #expect(uploadedProfileIDs == ["default", "other-profile"])
        #expect(getCount == 1)
        #expect(model.listeningHistoryEntries.count == 3)
        #expect(model.activeProfileListeningHistoryEntries.count == 2)
        #expect(model.listeningHistoryEntries.contains {
            $0.syncProfileID == "other-profile" && $0.listenedSeconds == 77
        })
        let mergedLocal = try #require(model.activeProfileListeningHistoryEntries.first {
            $0.networkEventID == localEntry.networkEventID
        })
        #expect(mergedLocal.trackID == importedTrack.id)
        #expect(mergedLocal.listenedSeconds == 45)
        let remoteOnly = try #require(model.activeProfileListeningHistoryEntries.first {
            $0.networkEventID == remoteOnlyEventID
        })
        #expect(remoteOnly.title == "Remote-only title")
        #expect(remoteOnly.artist == "Remote-only artist")
        ...
        #expect(relaunched.activeProfileListeningHistoryEntries.contains {
            $0.networkEventID == remoteOnlyEventID
                && $0.title == "Remote-only title"
        })
    }
