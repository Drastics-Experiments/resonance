...
struct MacSourceImportResponse: Decodable, Sendable {
...
struct MacSourceImportDuplicateResponse: Decodable, Sendable {
    enum Status: String, Decodable, Sendable {
        case duplicate
    }
...
enum MacSourceImportPolicy {
    private static let canonicalPrefix = "https://www.youtube.com/watch?v="

    static func exactCanonicalYouTubePage(_ rawUserInput: String?) -> URL? {
        guard let rawUserInput,
              rawUserInput.hasPrefix(canonicalPrefix) else { return nil }
        let videoID = rawUserInput.dropFirst(canonicalPrefix.count)
        guard videoID.count == 11,
              videoID.unicodeScalars.allSatisfy({
                  CharacterSet.alphanumerics.union(CharacterSet(charactersIn: "-_")).contains($0)
              }),
              let url = URL(string: rawUserInput),
              url.absoluteString == rawUserInput else { return nil }
        return url
    }

    static func duplicateSong(from data: Data) -> RemoteSong? {
        guard let response = try? JSONDecoder().decode(MacSourceImportDuplicateResponse.self, from: data),
              response.schemaVersion == 1 else { return nil }
        return response.duplicateOf
    }
}
